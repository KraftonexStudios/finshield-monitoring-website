...
const BEHAVIORAL_SESSIONS_COLLECTION = 'behavioralSessions';
const TRANSACTIONS_COLLECTION = 'transactions';

// User operations
export const addUser = async (userData: Omit<User, 'id' | 'pinHash' | 'recoveryQuestions' | 'biometricEnabled' | 'biometricType' | 'createdAt' | 'updatedAt' | 'isActive' | 'lastLoginAt' | 'fcmToken'>): Promise<string> => {
  try {
...
export const addUserBehavioralProfile = async (profile: UserBehavioralProfile) => {
  try {
    const profileDoc = {
      ...profile,
    ...
    const docRef = await addDoc(collection(db, USER_BEHAVIORAL_PROFILES_COLLECTION), profileDoc);
...
export const updateUserBehavioralProfile = async (profileId: string, updates: Partial<UserBehavioralProfile>) => {
  try {
...
// Real-time listeners
export const subscribeToUserBehavioralProfiles = (callback: (profiles: any[]) => void) => {
  const q = query(
    collection(db, USER_BEHAVIORAL_PROFILES_COLLECTION),
    orderBy('lastUpdated', 'desc')
  );
...
export const addBehavioralSession = async (session: BehavioralSession) => {
  try {
    ...
    const docRef = await addDoc(collection(db, BEHAVIORAL_SESSIONS_COLLECTION), sessionDoc);
...
export const getBehavioralSessions = async (userId?: string) => {
  try {
    let q;
    if (userId) {
      q = query(
...
export const subscribeToUserBehavioralSessions = (userId: string, callback: (sessions: any[]) => void) => {
  const q = query(
    collection(db, BEHAVIORAL_SESSIONS_COLLECTION),
    orderBy('timestamp', 'desc')
  );
...
export const subscribeToAllBehavioralSessions = (callback: (sessions: any[]) => void) => {
  const q = query(
    collection(db, BEHAVIORAL_SESSIONS_COLLECTION),
    orderBy('timestamp', 'desc')
  );
...
export const addTransaction = async (transactionData: Omit<Transaction, 'id' | 'createdAt' | 'updatedAt'>): Promise<string> => {
  try {
    ...
    const docRef = await addDoc(collection(db, TRANSACTIONS_COLLECTION), transactionDoc);
    return docRef.id;
  } catch (error) {
